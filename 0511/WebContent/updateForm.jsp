...
<%
	...
	int releaseYear = release.get(Calendar.YEAR);
		...
		<div id="content">
		    ...
		    <div id="formBox">
		        ...
		        <input type="hidden" name="no" value="<%=no%>" />
		            <fieldset>
		                <legend class="screen_out">영화 수정폼</legend>
		                <div class="row">
		                    <label for="name">영화명</label>
		                    <input id="name" name="name" value="<%=movie.getName()%>" title="영화명을 입력해주세요" disabled placeholder="영화명 입력"/>
		                </div><!--//.row -->
		                <div class="row">
		                    <label for="director">감독</label>
		                    <input id="director" name="director" value="<%=movie.getDirector()%>" title="감독을 입력해주세요" disabled placeholder="감독 입력"/>
		                </div><!--//.row -->
		                <div class="row">
		                ...
		                <div class="row">
		                    ...
		                    <input id="audience_num" name="audienceNum" value="<%=movie.getAudienceNum()%>" title="관객수를 입력해주세요" disabled placeholder="관객수 입력"/>
		                    <em>명</em>
		                </div><!--//.row -->
		                <div class="row">
		                ...
		                <div class="row">
		                    ...
		                    <select id="movieRating" name="rating" title="관람가능 연령을 선택해주세요">
		                        <option 
		                        <% if(movie.getRating() == 'A') { %>
		                        selected
		                        <% } %>
		                        value="A">전체관람가</option>
		                        <option 
		                        <% if(movie.getRating() == 'B') { %>
		                        selected
		                        <% } %>
		                        value="B">12세 관람가</option>
		                        <option 
		                        <% if(movie.getRating() == 'C') { %>
		                        selected
		                        <% } %>
		                        value="C">15세 관람가</option>
		                        <option 
		                        <% if(movie.getRating() == 'D') { %>
		                        selected
		                        <% } %>
		                        value="D">19세 관람가</option>
		                    </select>
		                </div><!--//.row -->
		                <div id="btnBox">
		                    <button type="submit" class="btn">
		                        <i class="fa fa-floppy-o"></i> 영화 수정
		                    </button>
